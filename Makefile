...
LDFLAGS = $(CPU) -nostartfiles -Tldscripts/$(LDSCRIPT) -Wl,-Map=$(PROJECT).map,--cref,--gc-sections


#ASRC = start_705x.s
ASRC = start_mmc.s

SRC = cmd_parser.c eep_funcs.c main.c crc.c FLASH.c ISR.c SERIAL.c WDT.c CPU_STUFF.c
...
ifeq ($(BUILDWHAT), SH7051)
	...
	SRC += SH7052/CPU.c SH7052/FLASH_CONST.c
else
